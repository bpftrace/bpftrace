#!/usr/bin/env python3
"""
Python script to read each line of a file at a relative path.
"""
...
def parse_param_string(param_string: str) -> Param | None:
    """
...
def parse_last_expr_string(param_string: str) -> LastExpr | None:
    """
    Parse a parameter string in the format:
...
def parse_macro_name(macro_string: str) -> str | None:
...
def main():
    if len(sys.argv) != 2:
        print("Usage: generate_stdlib_docs.py <relative_file_path>")
        print("Example: python generate_stdlib_docs.py src/stdlib/base.bt")
        sys.exit(1)

    file_path = sys.argv[1]
    print(f"Reading file: {file_path}")
    macros = read_file_lines(file_path)

    if macros:
        write_markdown_doc(macros)
    else:
        sys.exit(1)
