...
def parse_bpf_helper(line: str) -> Optional[str]:
    pattern = r':bpf_helper\s+(.+)'
        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            for line in file:
                ...
                if line_content.startswith("//"):
                    ...
                    elif line_content.startswith(":bpf_helper"):
                        parsed_bpf_helper = parse_bpf_helper(line_content)
                        if parsed_bpf_helper:
                            current.bpf_helper = parsed_bpf_helper
    ...
    for cleaned_line in cleaned_lines:
        ...
        if line == "## Helpers":
            ...
            for helper in helpers:
                ...
                if helper.bpf_helper:
                    updated_lines.append(f"**bpf helper:** `{helper.bpf_helper}`\n")
                updated_lines.append("\n")
        else:
            updated_lines.append(cleaned_line)
