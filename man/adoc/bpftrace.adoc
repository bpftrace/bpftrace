...
| Create a time series. tseries can be combined with `count`, `sum`, `avg`, `min`, or `max` to keep track of the count, sum, average, min, max of something in each interval.
| Sync
...
`interval`. Its behavior depends on the type of `v`.
