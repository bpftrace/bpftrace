...
*bpftrace* `doc` [_OPTIONS_] _FILENAME_...
...
Extract the documentation for a stdlib helper::
----
# bpftrace doc --name cgroup src/stdlib/base.bt
----
...
=== *doc* [_OPTIONS_] _FILENAME_...

Extract documentation comments for macros, functions, and probes from the provided input files.
When `--name` is specified only matching entries are emitted.

The available options are::
*-n, --name* _NAME_ to filter by a specific macro, function, or probe +
*-o, --output* _FILE_ to write the extracted markdown to _FILE_ +
*-h, --help* to print the subcommand help
...
In addition, it makes kprobe/kretprobe checks less restrictive.
It can be used to probe functions that bpftrace reports as not traceable, but are supported if the kernel is configured to allow probing notrace functions.
