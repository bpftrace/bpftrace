...
This will override the default behavior, which prints maps when a `SIGUSR1` is received. See the <<Map Printing>> section for more details.
...
Note: this behavior will be overriden by defining custom signal handling probes.
For more details see the <<probes-bpftrace, `bpftrace`>> section.
