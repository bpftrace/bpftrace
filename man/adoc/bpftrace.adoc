...
This manpage contains numerous terms. See the *Terminology* section for more information about these terms.

== Examples

=== Running a one-liner program
----
...
See the *Options* section for more information on the command line options.

The syntax of the one-liner program will be explained in the *bpftrace Language* section.

A program will continue running until Ctrl-C is hit, or an exit() function is called.

=== Running a bpftrace script
...
Start by adding an interpreter line at the top (`#!`) with either the path to your installed bpftrace (/usr/local/bin is the default) or the path to `env`
...
----
  ...
  [...]
----
The *-v* option allows more details to be printed, see the *Listing Probes* section for more information.

=== Trace processes calling sleep
This example is printing when processes call the nanosleep syscall.
----
...
We can also trace processes calling sleep while spawning `sleep 5` as a child process:
----
  # bpftrace -e 'kprobe:do_nanosleep { printf("%d sleeping\n", pid); }' -c 'sleep 5'
----
...
==== *-I* _DIR_
The `-I` option can be used to add directories _DIR_ to the list of directories that bpftrace uses to look for
