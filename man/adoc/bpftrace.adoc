...
Amount of bpftrace stack (in bytes) to allocate per CPU.

Constructs like scratch values and compiler generated temporaries will be placed on the bpftrace stack. This bypasses
the verifier's 512 byte limit on the built-in stack. Note that LLVM may still spill registers onto the built-in stack.
But the larger (and more problematic) stack allocations will go onto the bpftrace stack.
