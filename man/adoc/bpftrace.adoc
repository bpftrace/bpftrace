...
* `fentry[:module]:fn`
* `fexit[:module]:fn`
* `fentry:bpf[:prog_name]:prog_id`
* `fexit:bpf[:prog_name]:prog_id`
...
The bpf variants (e.g. `fentry:bpf[:prog_name]:prog_id`) allow attaching to running BPF programs and sub programs.
You can specify just the program name and in this case bpftrace will attach to all running programs with that name (differentiate between them using the `probe` builtin).
You can also specify the program id (e.g. `fentry:bpf:*:123` or just `fentry:bpf:123`) to attach to a specific running BPF program.
You can see a list of running, valid BPF programs and sub programs with `bpftrace -l 'fentry:bpf:*'`. Note: only BPF programs with a BTF Id can be attached to.
...
| <<functions-socket_cookie, `socket_cookie`>>
| Get the cookie of a socket
| Sync
