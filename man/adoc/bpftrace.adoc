...
For a single value map, the key may be omitted.
For multi-key maps, wildcards can be used to delete multiple map elements. In such a case, a single concrete value must be always specified.
...
k:dummy {
  ...
  @map[1,3] = 3;
  delete(@scalar);
  delete(@map[*,2]); // deletes @map[1,2] and @map[2,2]
  delete(@map[1,3]);
