...
A config variable can also be used to suppress warnings e.g. "log_warnings=0".
...
Print warnings to the log. If set to 0, this is the same as using the `--no-warnings` CLI option.
The reason to use this is to not always have to remember to use the CLI option if there are expected
runtime or compile warnings.
