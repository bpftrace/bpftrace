...
* `tseries_t tseries(int64 v, const string interval, int64 buckets, const string agg)`

Create a time series that tracks an integer value. `tseries` records up to
`buckets` intervals where each bucket represents an amount of time specified by
`interval`.
...
By default, each bucket in `tseries` contains the last value recorded for that
interval. The optional `agg` parameter specifies how values in the same
interval are aggregated.
