...
All positional and named parameters should come after a double dash e.g.
```
# bpftrace -e 'BEGIN { ... }' -- p1 --aa=20 --bb p2
```
In this example there are two positional parameters (p1, p2) and two named parameters (aa - set to 20 and bb - set to true).
Named program parameters require the `=` unless they are boolean parameters (like 'bb' above).
...
| <<functions-getopt, `getopt`>>
| Get named command line option/parameter
| Sync
...
If no default type is provided, the option is treated like a boolean arg e.g. `getopt("hello")` would evaluate to `0` if `--hello` is not specified on the command line or `1` if `--hello` is passed or set to one of the following values: `1`, `true`, `on`, `yes`.
Additionally, boolean args accept the following false values: `0`, `false`, `off`, `no` e.g. `--hello=false`.
If the arg is not set on the command line, the default value is used.

```
# bpftrace -e 'BEGIN { print((getopt("aa", 10), getopt("bb", "hello"), getopt("cc"))); }' -- --cc --bb=bye
