...
function(bpf NAME)
  ...
  # kernel happy path.
  #
  # But at least what we built above has the DWARF data, right? Wrong! If were
  # to link the full binary, too much gets stripped out to generate the full
  # set of BTF information. So we need to generate just the object, add BTF
  # information, and separately link that single object into a binary to have
  # the DWARF data in a ELF binary that we're looking for.
  #
  ...
  add_custom_command(
    ...
    COMMAND gcc -g -I ${CMAKE_CURRENT_BINARY_DIR} -c ${CMAKE_CURRENT_SOURCE_DIR}/${ARG_SOURCE} -o ${CMAKE_CURRENT_BINARY_DIR}/${ARG_OBJECT} &&
            cmake -E env LLVM_OBJCOPY=${LLVM_OBJCOPY} ${PAHOLE} -J ${CMAKE_CURRENT_BINARY_DIR}/${ARG_OBJECT}
    VERBATIM
  )
  ...
  if (DEFINED ARG_BINARY)
    add_custom_command(
      ...
      COMMAND gcc -g -o ${CMAKE_CURRENT_BINARY_DIR}/${ARG_BINARY} ${CMAKE_CURRENT_BINARY_DIR}/${ARG_OBJECT} &&
              cmake -E env LLVM_OBJCOPY=${LLVM_OBJCOPY} ${PAHOLE} -J ${CMAKE_CURRENT_BINARY_DIR}/${ARG_BINARY}
      VERBATIM
    )
