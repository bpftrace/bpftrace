...
| --- | --- | --- | --- |
| [`$1`, `$2`, `...$n`](#positional-parameters) | int64 | n/a | The nth positional parameter passed to the bpftrace program. If less than n parameters are passed this evaluates to `0` in an action block or an empty string in a probe. For string arguments in an action block use the `str()` call to retrieve the value. |
...
Functions that are marked **async** are asynchronous which can lead to unexpected behaviour, see the [Invocation Mode](../man/adoc/bpftrace.adoc#invocation-mode) section for more information.
...
When available (starting from kernel 5.5, see the `--info` flag) bpftrace will automatically use the `kernel` or `user` variant of `probe_read_{kernel,user}_str` based on the address space of `data`, see [Address-spaces](../man/adoc/bpftrace.adoc#Address-spaces) for more information.
...
Functions that are marked **async** are asynchronous which can lead to unexpected behavior, see the [Invocation Mode](../man/adoc/bpftrace.adoc#invocation-mode) section for more information.

See [Advanced Topics](../man/adoc/bpftrace.adoc#advanced-topics) for more information on [Map Printing](../man/adoc/bpftrace.adoc#map-printing).
