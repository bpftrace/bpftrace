...
### find
- `boolean find(map m, mapkey k, mapvalue var)`

Return `true` if the key exists in this map and sets the passed scratch variable (var) to the value of that map key.
Otherwise return `false` and don't mutate var.
Use this instead of `has_key` + a map access to avoid an additional map lookup.
