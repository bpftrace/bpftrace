...
The `RDParser` class is a single-pass recursive descent parser that transforms bpftrace source text into an AST. There is no separate lexer/tokenizer phase.
...
```
RDParser::parse()      -> ast::Program*    // Full program parse
RDParser::parse_expr() -> optional<Expression>  // Single expression (used by CMacroExpander)
```
...
`parse_sized_type` handles type syntax: `int32`, `string[64]`, `struct foo *`, `void`, etc. It supports arrays (`type[N]`), pointers
(`type *`), and composite type keywords (`struct`, `union`, `enum`).

`try_parse_sized_type` is a non-destructive version that uses `SavePoint` to rewind on failure, used in cast disambiguation and `typeof` parsing.
...
`skip_whitespace()` handles `//` line comments, `/* */` block comments, and records both comments and vertical whitespace (blank lines) in the `ASTContext` via `ctx_.add_comment(loc)` and `ctx_.add_vspace(loc, count)`. This information is used by the formatter (`bpftrace --fmt`) to preserve comment placement and blank-line structure. Vertical space is suppressed after `{`, `}`, and `;` to avoid double-spacing.
...
2. **Add a `parse_*` method** to `RDParser` (declare in the header, define
   in the `.cpp`).
