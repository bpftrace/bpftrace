...
1. Mark the release in [CHANGELOG.md](../CHANGELOG.md) by replacing the `##
   Unreleased` header with `## [0.Y.0] TBD`.
1. Create a new branch according to the [Branching model](#branching-model).
1. In master, increment `bpftrace_VERSION_MINOR` in
   [CMakeLists.txt](../CMakeLists.txt) to `Y+1`. Once merged, tag the
   corresponding commit with `v0.<Y+1>.0-init`.
...
1. Mark the release in [CHANGELOG.md](../CHANGELOG.md) by replacing `TBD` by the
   current date in the corresponding header.
...
1. Forward-port the CHANGELOG.md changes from the release branch to master. This
   applies to CHANGELOG.md entries which were backported from master after the
   release branch was cut.
1. Increment `bpftrace_VERSION_PATCH` in [CMakeLists.txt](../CMakeLists.txt) in
   the release branch.
