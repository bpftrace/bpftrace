...
In addition to a callback function, a Consumer consists of another `GraphNode` and after the callback returns we use this Consumer `GraphNode` as the next Source to lookup the next set of Consumers to call with the returned type in the graph/map.
...
1. `$a` (on the right side of `$b`) subscribes to the ScopedVariable `$a` (*notice the distinction between the variable `Node` and a `ScopedVariable`)
...
1. the callback sees that it has a `None` type (by looking it up in the `resolved_types_` map) so it can safely forward/return `uint8`
1. the `propagate_resolved_types` logic gets the `uint8` from the returned Consumer callback and sets this as the value of the Consumer `GraphNode` in `resolved_types_`
1. it then looks to see if the graph has any Consumers for the ScopedVariable `$a` and finds the variable Node `$a`
1. the callback for Variable Node `$a` returns this new type
1. `propagate_resolved_types` sets the value for variable Node `$a` to `uint8` in the `resolved_types_` map
...
So in order to resolve the type of `$a` we need a full resolution of the TypeGraph visitor (minus the `comptime` branches). We then call upon the `IntrospectionFolder` to transform `typeinfo($a)` into a literal record (e.g. `(btf_id=0, base_ty="int", full_ty="uint64")`). We can then evaluate the comptime expression and, if it's part of a conditional in an `if` statement (like above), decide what branch to keep.

We then need to run the TypeGraph visitor again because there are new branches to visit. We basically have to keep doing this until we either have no more `comptime` expressions (as they can be nested) OR we reach a point where we simply can't resolve them (and return an error).
