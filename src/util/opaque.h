...
private:
  struct OwnedBuffer {
    OwnedBuffer(const char *data) : data(data) {};
    const char *data;
  };
  using SharedMemory = std::shared_ptr<OwnedBuffer>;
  using Variant = std::variant<uintptr_t, SharedMemory>;
...
public:
  ...
  {
    return OpaqueValue(sizeof(T) * length, [&](char *data) {
      memcpy(data,
  ...
  static OpaqueValue from(const T &other)
  {
    return OpaqueValue(sizeof(other), [&](char *data) {
...
private:
  ...
  OpaqueValue(Variant mem, size_t offset, size_t length)
