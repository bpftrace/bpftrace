...
// the modern versions, but still call it `Error` and `ErrorOr` to make its
// purpose clear. In the future, this could map to a newer standard, such as
// `std::expected<T, Error>`, independently of the names in this namespace.
//
// For now, the LLVM `Error` class provides mandatory checking (errors cannot
// be ignored, they must be explicitly forward or consumed).
template <typename T>
using ErrorOr = llvm::Expected<T>;

// success (the class) is used in order to indicate success (the concept).
//
...
//
// To indicate success, you can merely do `return success()`.
class success {};
