...
template <typename T>
std::pair<EpochType, uint64_t> reduce_tseries_value(const OpaqueValue &value,
                                                    TSeriesAggFunc agg)
{
  // Combine values only from the same epoch and return the reduced value from
  // the latest epoch in this bucket.
  std::map<EpochType, tseries_data<T>> epoch_to_value;
  EpochType latest_epoch = 0;
...
std::pair<EpochType, OpaqueValue> reduce_tseries_value(
