...
static const auto pr_set_pdeathsig_msg = "pr_set_pdeathsig() failed";
static const auto failed_event_msg = "failed to read event fd";
static const auto traceme_msg = "child: ptrace(traceme) failed";
static const auto stop_msg = "child: failed to stop";
static const auto execve_msg = "child: failed to execve";
...
int ChildProcImpl::childfn(void* arg)
{
  ...
  if (prctl(PR_SET_PDEATHSIG, SIGTERM)) {
    write(args->result_fd, pr_set_pdeathsig_msg, sizeof(pr_set_pdeathsig_msg));
    return 1;
  }
  ...
  if (read(args->command_fd, &command, sizeof(command)) < 0) {
    int err = errno;
    write(args->result_fd, failed_event_msg, sizeof(failed_event_msg));
    return err;
  }
  switch (command) {
    case Go:
    ...
    case Ptrace:
      if (ptrace(PTRACE_TRACEME, 0, NULL, NULL) < 0) {
        int err = errno;
        write(args->result_fd, traceme_msg, sizeof(traceme_msg));
        return err;
      }
      if (kill(getpid(), SIGSTOP) < 0) {
        int err = errno;
        write(args->result_fd, stop_msg, sizeof(stop_msg));
        return -err;
      }
      break;
  }
  ...
  int err = errno;
  write(args->result_fd, execve_msg, sizeof(execve_msg));
  return err;
}
...
ChildProcImpl::~ChildProcImpl()
{
  if (!exit_code_ && !term_signal_) {
    ...
    if (!ok) {
      // Just ignore this error; since there is no way to propagate.
    }
  }
}
...
Result<> ChildProcImpl::run(bool pause)
{
  ...
  auto check_result = [this]() -> Result<> {
    ...
    if (n > 0) {
      return make_error<SystemError>(std::string("Child failed: ") + buf);
    }
    return OK();
  };
