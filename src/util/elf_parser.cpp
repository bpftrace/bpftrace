...
Result<std::vector<usdt_probe_entry>> USDTProbeEnumerator::enumerate_probes()
{
  ...
  while ((off = gelf_getnote(data, off, &nhdr, &name_off, &desc_off)) > 0) {
    auto note_res = parse_usdt_probe_entry(
        elf_path, &nhdr, (const char*)data->d_buf, name_off, desc_off);
    if (!note_res) {
      continue;
    }
    auto note = *note_res;
    ...
    if (note.sema_addr) {
      ...
      auto seg = std::find_if(
          segs.begin(), segs.end(), [note](const elf_seg& seg) {
  ...
  std::transform(unique_probes.begin(),
                 unique_probes.end(),
