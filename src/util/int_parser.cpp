...
Result<uint64_t> to_uint(const std::string &num, int base)
{
  ...
  if (ret == 0 && endptr == n.c_str()) {
    return make_error<NumberFormatError>("invalid integer", std::move(n));
  }
  if (ret == ULLONG_MAX && errno == ERANGE) {
    return make_error<OverflowError>(std::move(n), ULLONG_MAX);
  }
  ...
  if ((*endptr == 'e' || *endptr == 'E') && *(endptr + 1) >= '1' &&
    ...
    if (ret <= 0 || ret >= 10) {
      return make_error<NumberFormatError>(
          "coefficient part of scientific literal must be 1-9", std::move(n));
    }
    char *exp = endptr + 1;
    uint64_t exp_pow = std::strtoull(exp, &endptr, 10);
    if (exp_pow == 0 && endptr == exp) {
      return make_error<NumberFormatError>("invalid exponent", std::move(n));
    }
    // Compute the result, ensuring that we never overflow.
    auto maybe_result = safe_exp(ret, exp_pow);
    if (!maybe_result) {
      return make_error<OverflowError>(std::move(n),
  ...
  if (typespec == max.end()) {
    // Not a valid suffix.
    return make_error<NumberFormatError>("invalid trailing bytes",
                                         std::move(n));
  }
  // Is it out of the range? We consider this as an overflow.
  if (ret > typespec->second) {
    return make_error<OverflowError>(std::move(n), typespec->second);
  }
  return ret;
}
