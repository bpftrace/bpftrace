...
std::string AttachedProbe::eventname() const
{
  ...
  switch (probe_.type)
  {
    ...
    case ProbeType::rawtracepoint:
      ...
      return eventprefix() + sanitiseBPFProgramName(probe_.attach_point) + "_" +
    ...
    case ProbeType::usdt:
      offset_str << std::hex << offset_;
      return eventprefix() + sanitiseBPFProgramName(probe_.path) + "_" +
             offset_str.str() + index_str;
    case ProbeType::tracepoint:
...
void AttachedProbe::load_prog(BPFfeature &feature)
{
...

  {
    ...
    name = sanitiseBPFProgramName(name);
