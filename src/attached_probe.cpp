...
void AttachedProbe::resolve_offset_kprobe(bool safe_mode)
{
  ...
  if (!probe_.attach_point.empty() && probe_.func_offset == 0)
    return;

  // Setup the symbol to resolve, either using the address or the name.
  struct symbol sym = {};
  if (!probe_.attach_point.empty())
  ...
  if (!probe_.attach_point.empty()) {
    ...
    if (__builtin_sub_overflow(probe_.address, sym.start, &probe_.func_offset))
      throw FatalUserException("Offset before the function bounds ('" +
                               probe_.attach_point + "' address is " +
                               std::to_string(sym.start) + ")");
  }
  ...
  uint64_t sym_offset = resolve_offset(path.value(),
                                       probe_.attach_point,
                                       probe_.loc);
  check_alignment(path.value(),
                  probe_.attach_point,
                  sym_offset,
                  probe_.func_offset,
                  safe_mode,
...
void AttachedProbe::attach_kprobe(bool safe_mode)
{
  ...
  bool is_address_kprobe = probe_.attach_point.empty();
  resolve_offset_kprobe(safe_mode);
  if (is_address_kprobe)
