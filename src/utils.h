...
template <typename T>
std::pair<T,uint64_t> reduce_tseries_value(const std::vector<uint8_t> &values, int nvalues, const SizedType& inner_type)
{
  ...
  for (int i = 0; i < nvalues; i++) {
    ...
    if (epoch_to_value.find(epoch) == epoch_to_value.end()) {
      epoch_to_value[epoch] = std::pair<T, uint64_t>(
        read_data<T>(val),
        meta
      );
