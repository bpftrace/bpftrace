  ...
  while (i < expr_u8.size()) {
    ...
    switch (op) {
      ...
      case llvm::dwarf::DW_OP_const1s:
        v = (uint64_t)(int64_t)(int8_t)(uint8_t)expr_get(expr_u8, 1, i);
        append_u8(eout, EXPR_OP_CONST);
        append_u64(eout, v);
        break;
      ...
      case llvm::dwarf::DW_OP_const2s:
        v = (uint64_t)(int64_t)(int16_t)(uint16_t)expr_get(expr_u8, 2, i);
        append_u8(eout, EXPR_OP_CONST);
        append_u64(eout, v);
        break;
      ...
      case llvm::dwarf::DW_OP_const4s:
        v = (uint64_t)(int64_t)(int32_t)(uint32_t)expr_get(expr_u8, 4, i);
        append_u8(eout, EXPR_OP_CONST);
        append_u64(eout, v);
        break;
      ...
      default:
        LOG(ERROR) << "DWARF expression op 0x" << std::hex <<
          static_cast<int>(op) << std::dec << " not implemented yet.";
    ...
    std::map<uint64_t, uint64_t> &map_offsets)
{
  for (const auto &Section : Obj->sections()) {
    const typename ELFT::Shdr *header =
      Obj->getSection(Section.getRawDataRefImpl());
  ...
  const std::map<uint64_t, uint64_t> &map_offsets)
{
      ...
      for (const auto &row : *table) {
        ...
        switch (cfa.getLocation()) {
          ...
          case llvm::dwarf::UnwindLocation::DWARFExpr: {
            auto expr = *cfa.getDWARFExpressionBytes();
            auto expr_id = add_expression(expr);
            append_u32(s, 2);
        ...
        for (uint32_t reg : locations.getRegisters()) {
          auto loc = *locations.getRegisterLocation(reg);
          ...
          switch (loc.getLocation()) {
            ...
            case llvm::dwarf::UnwindLocation::CFAPlusOffset:
              if (loc.getDereference())
                append_u32(rs, 3);   // val_offset(N)
              else
                append_u32(rs, 4);   // offset(N)
              append_u64(rs, loc.getOffset());
              break;
            case llvm::dwarf::UnwindLocation::RegPlusOffset:
              if (loc.getOffset() != 0) {
                LOG(V1) <<
                     "RegPlusOffset with non-zero offset not supported for reg "
                        << reg;
                return DWARFError::ParseError;
              }
  ...
  LOG(V1) << "expressions: " << expressions_.size() << ", entries: "
          << entries_.size() << ", rows: " << rows.size();

  std::vector<std::pair<uint64_t, uint64_t>> table_entries;
  for (const auto &row : rows) {
    table_entries.push_back({ row.first, row.second });
  }
  size_t start = 0;
  while (start < table_entries.size()) {
    ...
    e.first->second.push_back(TableMapping {
      .file_offset = table_entries[start].first,
    ...
    if (current_table_.empty()) {
      ...
      current_table_.insert(current_table_.end(),
                            table_data.begin(), table_data.end());
    }
...
std::vector<ProcessMapEntry> read_process_maps(int pid)
{
  ...
  while (std::getline(file, line)) {
    ...
    if (iss >> std::hex >> start >> dash >> end
        >> perms >> offset >> dev >> std::dec >> inode) {
      iss >> std::ws;
