...
const std::regex FormatSpec::regex(
    R"(%(-?)(\+?)( ?)(#?)(\*|\d+)?(?:\.(\*|\d+))?([hlLjzt]*)([diouxXeEfFgGaAcspn%]|r|rh|rx))");
...
void FormatString::parse()
{
  ...
  for (auto it = begin; it != end; ++it) {
    fragments.emplace_back(
        fmt_.substr(last_match_end, it->position() - last_match_end));
    specs.push_back(FormatSpec(*it));
    last_match_end = it->position() + it->length();
  }
  fragments.emplace_back(
      fmt_.substr(last_match_end, fmt_.size() - last_match_end));
}
  ...
  return std::visit(
      [&](const auto& v) -> Result<> {
        if constexpr (std::is_same_v<std::decay_t<decltype(v)>, int64_t> ||
                      std::is_same_v<std::decay_t<decltype(v)>, uint64_t> ||
                      std::is_same_v<std::decay_t<decltype(v)>, double> ||
                      std::is_same_v<std::decay_t<decltype(v)>, bool>) {
          if constexpr (std::is_same_v<T, void*>) {
            ss << reinterpret_cast<void*>(static_cast<unsigned long long>(v));
          } else if constexpr (std::is_same_v<T, char>) {
            ss << static_cast<int64_t>(static_cast<T>(v));
          } else if constexpr (std::is_same_v<T, unsigned char>) {
            ss << static_cast<uint64_t>(static_cast<T>(v));
          } else {
            ss << static_cast<T>(v);
          }
          return OK();
                           ...
                           const std::string& length_modifier)
{
  if (length_modifier == "hh") {
    return as_number<char>(ss, p);
  } else if (length_modifier == "h") {
    return as_number<short>(ss, p);
  } else if (length_modifier == "l") {
    return as_number<long>(ss, p);
  } else if (length_modifier == "ll") {
    return as_number<long long>(ss, p);
  } else if (length_modifier == "j") {
    return as_number<intmax_t>(ss, p);
  } else if (length_modifier == "z") {
    return as_number<ssize_t>(ss, p);
  } else if (length_modifier == "t") {
    return as_number<ptrdiff_t>(ss, p);
  } else {
    return as_number<T>(ss, p);
  }
}
...
{
  if (length_modifier == "hh") {
    return as_number<unsigned char>(ss, p);
  } else if (length_modifier == "h") {
    return as_number<unsigned short>(ss, p);
  } else if (length_modifier == "l") {
    return as_number<unsigned long>(ss, p);
  } else if (length_modifier == "ll") {
    return as_number<unsigned long long>(ss, p);
  } else if (length_modifier == "j") {
    return as_number<uintmax_t>(ss, p);
  } else if (length_modifier == "z") {
    return as_number<size_t>(ss, p);
  } else if (length_modifier == "t") {
    return as_number<ptrdiff_t>(ss, p);
  } else {
    return as_number<T>(ss, p);
                          ...
                          const std::string& length_modifier)
{
  if (std::holds_alternative<Primitive::Buffer>(p.variant)) {
    auto& buf = std::get<Primitive::Buffer>(p.variant);
