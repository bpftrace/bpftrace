...
{
  OS << "command line option --" << param_ << " " << err_
     << " (value: " << value_ << ")\n";
}
                                 ...
                                 const GlobalVarValue &default_val)
{
  ...
  added_global_vars_[name] = GlobalVarConfig({
      ...
      .type = std::holds_alternative<std::string>(default_val) ? Type::string
              : std::holds_alternative<bool>(default_val)      ? Type::boolean
                                                               : Type::integer,
  });
  ...
  for (const auto &[name, default_val] : named_param_defaults_) {
    ...
    if (config.type == Type::integer) {
      try {
        ...
        return make_error<NamedParamError>(name, val, "expects an integer");
      } catch (const std::out_of_range &) {
        return make_error<NamedParamError>(name, val, "value is out of range");
      }
    } else if (config.type == Type::boolean) {
