...
char UnknownOptsError::ID;
void UnknownOptsError::log(llvm::raw_ostream &OS) const
{
  OS << err_ << "\n";
}
    ...
    const GlobalVarVals &global_var_vals)
{
  ...
  for (auto &[global_var, offset] : vars_and_offsets) {
    auto it = global_var_vals.find(global_var);
    if (it != global_var_vals.end()) {
      auto &config = added_global_vars.at(global_var);
      if (config.type == GlobalVarType::integer ||
        ...
        *var = std::get<int64_t>(it->second);
        ...
        const std::string &val = std::get<std::string>(it->second);
                                 ...
                                 const std::string &default_val)
{
  ...
  added_global_vars_[name] = GlobalVarConfig({
      ...
      .type = type.IsStringTy() ? GlobalVarType::string
    ...
    std::unordered_map<std::string, std::string> named_args)
{
  ...
  for (const auto &[name, default_val] : named_param_defaults_) {
    ...
    if (config.type == GlobalVarType::integer) {
      try {
        ...
        return make_error<CliOptsError>(
            "Command line option --" + name +
            " is expecting an integer. Got: " + val);
      } catch (const std::out_of_range &) {
        return make_error<CliOptsError>("Value for command line option --" +
                                        name + " is out of range. Got: " + val);
      }
  ...
  for (const auto &[name, val] : named_args) {
  ...
  if (!unexpected_args_str.empty()) {
    ...
    if (expected_args_str.empty()) {
      return make_error<UnknownOptsError>("Unexpected command line options: " +
                                              unexpected_args_str,
                                          "");
    }
    ...
    return make_error<UnknownOptsError>(
