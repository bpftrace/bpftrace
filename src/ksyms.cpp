  ...
  for (int j = static_cast<int>(sym->inlined_cnt) - 1; j >= 0; j--) {
    ...
    if (inlined != nullptr) {
      str_syms.push_back(stringify_ksym(
          inlined->name, &inlined->code_info, 0, 0, false, true));
    }
  }

  str_syms.push_back(stringify_ksym(
      sym->name, &sym->code_info, addr, sym->addr, show_offset, false));
...
std::vector<std::string> Ksyms::resolve_blazesym(uint64_t addr,
                                                 ...
                                                 bool show_debug_info)
{
  auto syms = resolve_blazesym_impl(addr, show_offset, show_debug_info);
  if (syms.empty()) {
                                        ...
                                        bool show_debug_info)
{
  ...
  if (config_.use_blazesym)
    return resolve_blazesym(addr, show_offset, show_debug_info);
#endif
