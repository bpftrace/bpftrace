...
{
  // clang-format off
  std::out << "USAGE:" << std::endl;
  std::out << "    bpftrace [options] filename" << std::endl;
  std::out << "    bpftrace [options] - <stdin input>" << std::endl;
  std::out << "    bpftrace [options] -e 'program'" << std::endl;
  std::out << std::endl;
  std::out << "OPTIONS:" << std::endl;
  std::out << "    -B MODE        output buffering mode ('full', 'none')" << std::endl;
  std::out << "    -f FORMAT      output format ('text', 'json')" << std::endl;
  std::out << "    -o file        redirect bpftrace output to file" << std::endl;
  std::out << "    -e 'program'   execute this program" << std::endl;
  std::out << "    -h, --help     show this help message" << std::endl;
  std::out << "    -I DIR         add the directory to the include search path" << std::endl;
  std::out << "    --include FILE add an #include file before preprocessing" << std::endl;
  std::out << "    -l [search|filename]" << std::endl;
  std::out << "                   list kernel probes or probes in a program" << std::endl;
  std::out << "    -p PID         enable USDT probes on PID" << std::endl;
  std::out << "    -c 'CMD'       run CMD and enable USDT probes on resulting process" << std::endl;
  std::out << "    --usdt-file-activation" << std::endl;
  std::out << "                   activate usdt semaphores based on file path" << std::endl;
  std::out << "    --unsafe       allow unsafe builtin functions" << std::endl;
  std::out << "    -q             keep messages quiet" << std::endl;
  std::out << "    --info         Print information about kernel BPF support" << std::endl;
  std::out << "    -k             emit a warning when a bpf helper returns an error (except read functions)" << std::endl;
  std::out << "    -kk            check all bpf helper functions" << std::endl;
  std::out << "    -V, --version  bpftrace version" << std::endl;
  std::out << "    --no-warnings  disable all warning messages" << std::endl;
  std::out << std::endl;
  std::out << "TROUBLESHOOTING OPTIONS:" << std::endl;
  std::out << "    -v                      verbose messages" << std::endl;
  std::out << "    --dry-run               terminate execution right after attaching all the probes" << std::endl;
  std::out << "    -d STAGE                debug info for various stages of bpftrace execution" << std::endl;
  std::out << "                            ('all', 'ast', 'codegen', 'codegen-opt', 'libbpf', 'verifier')" << std::endl;
  std::out << "    --emit-elf FILE         (dry run) generate ELF file with bpf programs and write to FILE" << std::endl;
  std::out << "    --emit-llvm FILE        write LLVM IR to FILE.original.ll and FILE.optimized.ll" << std::endl;
  std::out << std::endl;
  std::out << "ENVIRONMENT:" << std::endl;
  std::out << "    BPFTRACE_BTF                      [default: none] BTF file" << std::endl;
  std::out << "    BPFTRACE_CACHE_USER_SYMBOLS       [default: auto] enable user symbol cache" << std::endl;
  std::out << "    BPFTRACE_CPP_DEMANGLE             [default: 1] enable C++ symbol demangling" << std::endl;
  std::out << "    BPFTRACE_DEBUG_OUTPUT             [default: 0] enable bpftrace's internal debugging outputs" << std::endl;
  std::out << "    BPFTRACE_KERNEL_BUILD             [default: /lib/modules/$(uname -r)] kernel build directory" << std::endl;
  std::out << "    BPFTRACE_KERNEL_SOURCE            [default: /lib/modules/$(uname -r)] kernel headers directory" << std::endl;
  std::out << "    BPFTRACE_LAZY_SYMBOLICATION       [default: 0] symbolicate lazily/on-demand" << std::endl;
  std::out << "    BPFTRACE_LOG_SIZE                 [default: 1000000] log size in bytes" << std::endl;
  std::out << "    BPFTRACE_MAX_BPF_PROGS            [default: 512] max number of generated BPF programs" << std::endl;
  std::out << "    BPFTRACE_MAX_CAT_BYTES            [default: 10k] maximum bytes read by cat builtin" << std::endl;
  std::out << "    BPFTRACE_MAX_MAP_KEYS             [default: 4096] max keys in a map" << std::endl;
  std::out << "    BPFTRACE_MAX_PROBES               [default: 512] max number of probes" << std::endl;
  std::out << "    BPFTRACE_MAX_STRLEN               [default: 64] bytes on BPF stack per str()" << std::endl;
  std::out << "    BPFTRACE_MAX_TYPE_RES_ITERATIONS  [default: 0] number of levels of nested field accesses for tracepoint args" << std::endl;
  std::out << "    BPFTRACE_PERF_RB_PAGES            [default: 64] pages per CPU to allocate for ring buffer" << std::endl;
  std::out << "    BPFTRACE_STACK_MODE               [default: bpftrace] Output format for ustack and kstack builtins" << std::endl;
  std::out << "    BPFTRACE_STR_TRUNC_TRAILER        [default: '..'] string truncation trailer" << std::endl;
  std::out << "    BPFTRACE_VMLINUX                  [default: none] vmlinux path used for kernel symbol resolution" << std::endl;
  std::out << std::endl;
  std::out << "EXAMPLES:" << std::endl;
  std::out << "bpftrace -l '*sleep*'" << std::endl;
  std::out << "    list probes containing \"sleep\"" << std::endl;
  std::out << R"(bpftrace -e 'kprobe:do_nanosleep { printf("PID %d sleeping...\n", pid); }')" << std::endl;
  std::out << "    trace processes calling sleep" << std::endl;
  std::out << "bpftrace -e 'tracepoint:raw_syscalls:sys_enter { @[comm] = count(); }'" << std::endl;
  std::out << "    count syscalls by process name" << std::endl;
  // clang-format on
}
