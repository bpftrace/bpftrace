...
public:
  ...
  // Note that this returns a const version of this object, preventing any
  // additional creation.
...
public:
  ...
  Result<std::optional<FieldInfo>> field(const std::string &name) const;
  Result<std::map<std::string, FieldInfo>> fields() const;

private:
  static Result<Struct> add(HandleRef handle,
                            const std::string &name,
...
private:
  static Result<Union> add(HandleRef handle,
                           const std::string &name,
                           const std::map<std::string, ValueType> &fields);
  friend class BTF;
  friend std::ostream &operator<<(std::ostream & /*out*/,
...
// BTF represents a complete type system. It is typically constructed from a
// serialized BTF dataset, but may be constructed dynamically.
class BTF {
public:
  BTF() : handle_(std::make_shared<detail::Handle>()) {};
  BTF(const BTF &base)
      : handle_(std::make_shared<detail::Handle>(base.handle_)) {};

  BTF(BTF &&other) = default;
  BTF &operator=(BTF &&other) = default;
  BTF &operator=(const BTF &other) = delete;

  static Result<BTF> parse(const void *data, size_t sz);
  static Result<BTF> parse(const BTF &base, const void *data, size_t sz);
  ...
  Result<OK> relocate(const BTF &other);

  // Append another type system to this one.
  Result<OK> append(const BTF &other);
...
private:
  BTF(HandleRef &&handle) : handle_(std::move(handle)) {};
  HandleRef handle_;
};
