...
#define ERR_PTR_TOO_LARGE 1
#define ERR_VALUE_TOO_LARGE 2
...
static std::vector<uint8_t> enc_rel_key(int64_t rel_key)
{
  ...
  if (rel_key >= -111 && rel_key <= 111) {
    ...
    return {
      static_cast<uint8_t>(0xe0 | ((ukey >> 8) & 0x1f)),
...
static std::vector<uint8_t> enc_rel_ptr(uint64_t rel_ptr)
{
  if (rel_ptr <= 0xcf) {
    ...
    return {
      static_cast<uint8_t>(0xd0 | ((rel_ptr >> 16) & 0x07)),
      static_cast<uint8_t>(rel_ptr & 0xff),
      static_cast<uint8_t>(rel_ptr >> 8)
    };
  } else {
    throw ERR_PTR_TOO_LARGE;
  }
}
...
static std::vector<uint8_t> enc_rel_leaf_ptr(uint64_t rel_ptr)
{
  if (rel_ptr <= 0x1f) {
    ...
    return {
      static_cast<uint8_t>(0xd8 | ((rel_ptr >> 16) & 0x07)),
      static_cast<uint8_t>(rel_ptr & 0xff),
      static_cast<uint8_t>(rel_ptr >> 8)
    };
  } else {
...
static std::vector<uint8_t> enc_value(uint64_t val)
{
  if (val <= 0xf6) {
    ...
    return {
      static_cast<uint8_t>(0xf8 | ((val >> 8) & 0x07)),
    ...
    bool unmarked_leaf)
{
  ...
  if (len <= 3) {
    ...
    auto own_key = entries[start + 1].first;
    if (len == 1)
  ...
  // left subtree
  auto left_ptr = build_recurse(stats, buf, entries, start, split, own_key,
                                false);
  // right subtree
  build_recurse(stats, buf, entries, split + 1, end, own_key,
                right_unmarked_leaf);
    ...
    size_t start, size_t max_size, size_t& out_entries)
{
  ...
  while (left < right) {
    ...
    try {
      ...
      if (e != ERR_PTR_TOO_LARGE)
        throw std::runtime_error("table build failed");
    }
  ...
  std::reverse(result_buf.begin(), result_buf.end());
