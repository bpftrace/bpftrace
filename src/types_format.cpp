                             ...
                             uint32_t div)
{
  switch (type.GetTy()) {
    ...
    case Type::buffer: {
      auto *buf = reinterpret_cast<const AsyncEvent::Buf *>(value.data());
      ...
      for (auto &field : type.GetFields()) {
        std::vector<uint8_t> elem_data(value.begin() + field.offset,
                                       value.begin() + field.offset +
                                           field.type.GetSize());
        auto val = format(
            bpftrace, c_definitions, field.type, elem_data, top, 1, div);
        if (!val) {
          return val.takeError();
        }
        if (!std::holds_alternative<output::Primitive>(val->variant)) {
        ...
        record.fields.emplace_back(
            field.name, std::move(std::get<output::Primitive>(val->variant)));
      }
      return record;
    }
    case Type::tuple: {
    ...
    uint32_t k)
{
  ...
  for (int i = min_index; i <= max_index; i++) {
    ...
    if (i == 0) {
      hist.labels.push_back(0);
    } else if (i <= (2 << k)) {
      hist.labels.push_back(i);
    } else {
                             ...
                             uint32_t div)
{
  ...
  if (value_type.IsHistTy() || value_type.IsLhistTy()) {
    ...
    for (const auto &[key, count] : total_counts_by_key) {
      ...
      if (!std::holds_alternative<output::Primitive>(key_val->variant)) {
        return make_error<TypeFormatError>(key_type);
      }
      rval.values.emplace_back(std::move(std::get<output::Primitive>(
                                   key_val->variant)),
                               std::move(hist));
    }
  ...
  for (auto &[key, value] : *values_by_key) {
    ...
    auto val_res = format(
        bpftrace, c_definitions, value_type, value, 0, nvalues, div);
    if (!val_res) {
    ...
    if (map_info.is_scalar) {
      if (stats) {
        if (!std::holds_alternative<output::Primitive>(val_res->variant)) {
          return make_error<TypeFormatError>(value_type);
        }
        return output::Value(output::Value::Stats(
            std::move(std::get<output::Primitive>(val_res->variant))));
      }
    ...
    if (!std::holds_alternative<output::Primitive>(key_res->variant)) {
      return make_error<TypeFormatError>(key_type);
    }
    rval.values.emplace_back(std::move(
                                 std::get<output::Primitive>(key_res->variant)),
                             std::move(*val_res));
  }
