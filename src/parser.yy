...
%type <ast::Expression> non_if_expr expr unary_expr primary_expr prefix_expr postfix_expr
...
while_stmt:
                UNROLL unary_expr none_block { $$ = driver.ctx.make_node<ast::Unroll>($2, $3, @1 + @2); }
        |       WHILE  unary_expr none_block { $$ = driver.ctx.make_node<ast::While>($2, $3, @1); }
                ;
...
if_stmt:
                IF unary_expr none_block                 { $$ = driver.ctx.make_node<ast::IfExpr>($2, $3, driver.ctx.make_node<ast::None>(@1), @$); }
        |       IF unary_expr bare_block ELSE none_block { $$ = driver.ctx.make_node<ast::IfExpr>($2, $3, $5, @$); }
        |       IF unary_expr bare_block ELSE if_stmt    { $$ = driver.ctx.make_node<ast::IfExpr>($2, $3, $5, @$); }
                ;


if_expr:
                IF unary_expr block_expr ELSE if_expr    { $$ = driver.ctx.make_node<ast::IfExpr>($2, $3, $5, @$); }
        |       IF unary_expr block_expr ELSE block_expr { $$ = driver.ctx.make_node<ast::IfExpr>($2, $3, $5, @$); }
                ;
...
cast_expr:
        ...
        |       LPAREN type RPAREN unary_expr          { $$ = driver.ctx.make_node<ast::Cast>($2, $4, @1 + @3); }
/* workaround for typedef types, see https://github.com/bpftrace/bpftrace/pull/2560#issuecomment-1521783935 */
        ...
        |       LPAREN IDENT RPAREN unary_expr         { $$ = driver.ctx.make_node<ast::Cast>(ast::ident_to_record($2, 0), $4, @1 + @3); }
        |       LPAREN IDENT "*" RPAREN cast_expr      { $$ = driver.ctx.make_node<ast::Cast>(ast::ident_to_record($2, 1), $5, @1 + @4); }
        |       LPAREN IDENT "*" RPAREN unary_expr     { $$ = driver.ctx.make_node<ast::Cast>(ast::ident_to_record($2, 1), $5, @1 + @4); }
        |       LPAREN IDENT "*" "*" RPAREN cast_expr  { $$ = driver.ctx.make_node<ast::Cast>(ast::ident_to_record($2, 2), $6, @1 + @5); }
        |       LPAREN IDENT "*" "*" RPAREN unary_expr { $$ = driver.ctx.make_node<ast::Cast>(ast::ident_to_record($2, 2), $6, @1 + @5); }
                ;
