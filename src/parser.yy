...
shift_expr:
                addi_expr                  { $$ = $1; }
        |       shift_expr LEFT addi_expr  { $$ = new ast::Binop($1, token::LEFT, $3, @2); }
        |       shift_expr RIGHT addi_expr { $$ = new ast::Binop($1, token::RIGHT, $3, @2); }
                ;

muli_expr:
                cast_expr                  { $$ = $1; }
        |       muli_expr MUL cast_expr    { $$ = new ast::Binop($1, token::MUL, $3, @2); }
        |       muli_expr DIV cast_expr    { $$ = new ast::Binop($1, token::DIV, $3, @2); }
        |       muli_expr MOD cast_expr    { $$ = new ast::Binop($1, token::MOD, $3, @2); }
                ;

addi_expr:
                muli_expr                  { $$ = $1; }
        |       addi_expr PLUS muli_expr   { $$ = new ast::Binop($1, token::PLUS, $3, @2); }
        |       addi_expr MINUS muli_expr  { $$ = new ast::Binop($1, token::MINUS, $3, @2); }
                ;
