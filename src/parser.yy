...
%type <ast::Block *> none_block bare_block block_expr
%type <ast::Call *> call
...
%type <ast::Ternary *> if_stmt if_expr
...
expr_stmt:
                ...
                // `if_stmt` to avoid ambiguity. The `expr` node itself will accepts an
...
while_stmt:
                UNROLL "(" expr ")" none_block { $$ = driver.ctx.make_node<ast::Unroll>($3, $5, @1); }
...
if_stmt:
                IF "(" expr ")" none_block                 { $$ = driver.ctx.make_node<ast::Ternary>($3, $5, driver.ctx.make_node<ast::None>(@1), @$); }
        |       IF "(" expr ")" bare_block ELSE none_block { $$ = driver.ctx.make_node<ast::Ternary>($3, $5, $7, @$); }
        |       IF "(" expr ")" bare_block ELSE if_stmt    { $$ = driver.ctx.make_node<ast::Ternary>($3, $5, $7, @$); }
                ;


if_expr:
                IF "(" expr ")" block_expr ELSE if_expr    { $$ = driver.ctx.make_node<ast::Ternary>($3, $5, $7, @$); }
        |       IF "(" expr ")" block_expr ELSE block_expr { $$ = driver.ctx.make_node<ast::Ternary>($3, $5, $7, @$); }
                ;
...
primary_expr:
        ...
        |       NONE               { $$ = driver.ctx.make_node<ast::None>(@$); }
...
// This block is a bare block, but it allows for a final statement without a
// trailing semi-colon, simply for convenience. Always has none as a type.
//
// This is *always* bare to allow for disambigutiy with `if` rules.
bare_block:
                "{" stmt_list "}"
                {
                  auto *none = driver.ctx.make_node<ast::None>(@3);
                  $$ = driver.ctx.make_node<ast::Block>(std::move($2), none, @$);
                }
        |       "{" stmt_list nonexpr_stmt "}"
                {
                  ...
                  $$ = driver.ctx.make_node<ast::Block>(std::move(stmts), none, @$);
                }
                ;
