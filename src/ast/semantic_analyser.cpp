...
bool SemanticAnalyser::check_assignment(const Call &call, bool want_map, bool want_var, bool want_map_key)
{
  if (want_map && want_var && want_map_key)
  {
    ...
    {
      err_ << call.func << "() should be assigned to a map or a variable, or be used as a map key" << std::endl;
      return false;
    }
  }
  ...
  else if (want_map && want_map_key)
  {
    ...
    {
      err_ << call.func << "() should be assigned to a map or be used as a map key" << std::endl;
      return false;
    }
  }
  else if (want_var && want_map_key)
  {
    ...
    {
      err_ << call.func << "() should be assigned to a variable or be used as a map key" << std::endl;
      return false;
    }
  }
  else if (want_map)
  ...
  else
  {
    if (call.map || call.var || call.key_for_map)
    {
      err_ << call.func << "() should not be used in an assignment or as a map key" << std::endl;
      return false;
    }
  }
  return true;
