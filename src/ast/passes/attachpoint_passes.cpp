...
public:
  AttachPointChecker(BPFtrace &bpftrace,
...
AttachPointParser::State AttachPointParser::fentry_parser()
{
  ...
  if (parts_.size() == 3) {
    ...
    if (!util::has_wildcard(ap_->func)) {
      auto func_modules = func_info_state_.kernel_function_info().get_modules(
          ap_->func);
...
Pass CreateParseAttachpointsPass(bool listing)
{
  return Pass::create(
      "parse-attachpoints",
      [listing](ASTContext &ast, BPFtrace &b, FunctionInfo &func_info_state) {
        AttachPointParser ap_parser(ast, b, func_info_state, listing);
        ap_parser.parse();
      });
}
...
Pass CreateCheckAttachpointsPass(bool listing)
{
  return Pass::create(
      "check-attachpoints",
      [listing](ASTContext &ast, BPFtrace &b, FunctionInfo &func_info_state) {
        AttachPointChecker ap_checker(b, func_info_state, listing);
