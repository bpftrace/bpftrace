...
static bool is_comparison(const Expression& expr)
{
  auto* binop = expr.as<Binop>();
  if (!binop) {
    return false;
  }
  switch (binop->op) {
...
Buffer Formatter::format_list(std::vector<Expression>& exprs,
...
Buffer Formatter::visit(Tuple& tuple)
{
  ...
  auto elems = format_list(
      tuple.elems, ",", metadata, max_width - (bare ? 2 : 0));
  if (bare) {
    return elems;
  }
  if (elems.lines() > 1) {
...
Buffer Formatter::visit(AssignMapStatement& assignment)
{
  ...
  if (binop && binop->left.is<MapAccess>() &&
      *binop->left.as<MapAccess>() == *assignment.map) {
    ops = opstr(*binop) + "=";
    expr = format(binop->right,
                  metadata,
                  max_width - (map.width() + 2 + ops.size()),
