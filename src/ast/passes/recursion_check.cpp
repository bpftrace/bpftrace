...
public:
  explicit RecursionCheck(BPFtrace &bpftrace, FunctionInfo &func_info_state)
...
{
  // Create ProbeMatcher locally for this check
...
Pass CreateRecursionCheckPass()
{
  return Pass::create(
      "RecursionCheck",
      [](ASTContext &ast, BPFtrace &b, FunctionInfo &func_info_state) {
        auto recursion_check = RecursionCheck(b, func_info_state);
