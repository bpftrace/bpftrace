...
private:
  ...
  // Track meta expressions (sizeof, offsetof, typeof) to suppress warnings
  uint32_t meta_depth_ = 0;

  // Track variable assignment state for "use before assign" warnings
  std::vector<Node *> scope_stack_;
  std::map<Node *, std::map<std::string, bool>> variables_;
...
void TypeChecker::visit(Variable &var)
{
  ...
  if (auto *scope = find_variable_scope(var.ident)) {
    if (!variables_[scope][var.ident]) {
...
void TypeChecker::visit(VariableAddr &var_addr)
{
  ...
  if (auto *scope = find_variable_scope(var_addr.var->ident)) {
    variables_[scope][var_addr.var->ident] = true;
  }
}
...
void TypeChecker::visit(For &f)
{
  ...
  variables_[scope_stack_.back()][f.decl->ident] = true;
...
void TypeChecker::visit(AssignVarStatement &assignment)
{
  ...
  if (auto *scope = find_variable_scope(var_ident)) {
    variables_[scope][var_ident] = true;
  }
}
...
void TypeChecker::visit(VarDeclStatement &decl)
{
  ...
  if (!scope_stack_.empty()) {
    ...
    variables_[scope_stack_.back()].insert({ var_ident, false });
...
void TypeChecker::visit(Subprog &subprog)
{
  ...
  for (SubprogArg *arg : subprog.args) {
    variables_[scope_stack_.back()].insert({ arg->var->ident, true });
  }
...
void TypeChecker::check_variable_decls()
{
  for (const auto &pair : variables_) {
    for (const auto &[ident, was_assigned] : pair.second) {
      if (was_assigned) {
        continue;
      }
