...
Result<OK> ResolveImports::importC([[maybe_unused]] Node &node,
                                   const std::string &name,
                                   const std::filesystem::path &path,
                                   std::map<std::string, Data> &where)
{
  ...
  std::stringstream buf;
  buf << file.rdbuf();
  auto [_, added] = where.emplace(name, Data(buf.str()));
  assert(added);
  return OK();
}
                                   ...
                                   std::map<std::string, Data> &where)
{
  ...
  auto [_, added] = where.emplace(name, Data(contents));
  assert(added);
  return OK();
}
