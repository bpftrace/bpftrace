...
Pass CreateUSDTArgumentsPass()
{
  return Pass::create("USDTArguments", [](ASTContext &ast, BPFtrace &b, Imports &imports) -> Result<> {
    auto usdt_arguments = USDTArgumentLift(ast);
    usdt_arguments.visit(ast.root);
    if (usdt_arguments.needs_import()) {
      return imports.importAny(b, *ast.root, "stdlib/usdt");
    }
    return OK();
  });
};
