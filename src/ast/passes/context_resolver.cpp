  ...
  switch (probe_type) {
    ...
    case ProbeType::fexit: {
      ...
      return ResolvedContext{
        ...
        .type = CreatePointer(CreateRecord(std::shared_ptr<Struct>(*args)),
                              AddrSpace::none),
      };
    }
    case ProbeType::rawtracepoint: {
      auto args = bpftrace.btf_->resolve_raw_tracepoint_args(
          ap.func, func_info.kernel_function_info());
      if (!args)
        return args.takeError();
      return ResolvedContext{
    ...
    case ProbeType::tracepoint: {
      ...
      for (auto &field : (*args)->fields) {
        field.type.SetAS(addrspace);
      }
      return ResolvedContext{
        .args = nullptr, // Loosely typed, still don't convert.
        .type = CreatePointer(CreateRecord(std::move(*args)), AddrSpace::none),
      };
    }
      ...
      if (dwarf) {
        ...
        if (args) {
          ...
          return ResolvedContext{
            ...
            .type = CreatePointer(CreateRecord(
    ...
    case ProbeType::kretprobe: {
      auto args = bpftrace.btf_->resolve_args(
          ap.func, false, false, false, func_info.kernel_function_info());
      if (!args) {
        return ResolvedContext{
    ...
    case ProbeType::profile: {
      ...
      return ResolvedContext{
        .args = nullptr,
        .type = CreatePointer(
            CreateRecord(load_struct(bpftrace, "struct bpf_perf_event_data")),
            AddrSpace::none)
      };
    }
    case ProbeType::iter:
      ...
      return ResolvedContext{
        .args = nullptr,
        .type = CreatePointer(
            CreateRecord(load_struct(bpftrace, "struct bpf_iter__" + ap.func)),
            AddrSpace::none)
      };
    default:
  ...
  if (auto *builtin = expr.as<Builtin>()) {
    ...
    if (auto arg_num = builtin->argx()) {
      ...
      if (current_resolved_context_->type.IsPtrTy() &&
          current_resolved_context_->type.GetPointeeTy()->IsRecordTy() &&
          current_resolved_context_->type.GetPointeeTy()->HasField("$retval")) {
        // If we have BTF, we will have rewritten the return
        // value field to the special sentinel value `$retval`.
        auto new_expr = rewrite(*builtin, "$retval");
        expr.value = new_expr.value;
        return visit(expr);
      } else {
...
Expression ContextResolver::rewrite(Node &node, size_t field_index)
{
  ...
  if (!current_resolved_context_->args &&
      ...
      current_resolved_context_->type.GetPointeeTy()->IsRecordTy()) {
    const auto &fields =
        current_resolved_context_->type.GetPointeeTy()->GetFields();
