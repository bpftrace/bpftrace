...
public:
  ...
  {
    // If return was disallowed, it remains disallowed. However, if it
    // was allowed (from the top-level), then it continues to be allowed.
    ...
    for (size_t i = 0; i < block.stmts.size(); i++) {
      if (visit(block.stmts[i])) {
        ...
        if (!block.expr.is<None>()) {
          ...
          // Since the expression is unreachable, it is added to the list
          // dead_code as a statement, and the block becomes `none`.
        ...
        if (!dead_code.empty()) {
          // We rewrite the terminating statement to include the dead code
          // in an empty branch.
          //
