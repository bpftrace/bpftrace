...
void SemanticAnalyser::visit(Builtin &builtin)
{
  if (builtin.ident == "ctx") {
    ...
    auto task_struct_record = bpftrace_.structs.LookupOrAdd(
        "struct task_struct", 0);
    builtin.builtin_type = CreatePointer(CreateRecord("struct task_struct",
                                                      task_struct_record),
                                         AddrSpace::kernel);
...
void SemanticAnalyser::visit(FieldAccess &acc)
{
  ...
  if (!record) {
    acc.addError() << "Unable to resolve field \"" << acc.field << "\"";
