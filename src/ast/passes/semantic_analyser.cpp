                                         ...
                                         const SizedType &target_type)
{
  ...
  for (size_t i = 0; i < target_type.GetFields().size(); ++i) {
    ...
    if (auto *tuple_literal = exp.as<Tuple>()) {
      elem = clone(ctx_,
                   tuple_literal->elems.at(i).second.loc(),
                   tuple_literal->elems.at(i).second);
    } else {
    ...
    elems.emplace_back(target_type.GetField(i).name, std::move(elem));
  }
  ...
  if (auto *szof = expr.as<Sizeof>()) {
    ...
    if (field_access->expr.type().IsTupleTy()) {
      ...
      for (ssize_t i = 0; i < ty.GetFieldCount(); ++i) {
        const auto &field = ty.GetField(i);
