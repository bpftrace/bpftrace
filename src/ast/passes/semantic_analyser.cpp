...
void SemanticAnalyser::visit(For &f)
{
  ...
  variables_[scope_][decl_name] = { f.decl->type, true, true };
...
void SemanticAnalyser::visit(AssignVarStatement &assignment)
{
  ...
  if (search != variables_[scope_].end()) {
    ...
    variables_[scope_].insert({ var_ident, { assignTy, true, true } });
  }
...
void SemanticAnalyser::visit(VarDeclStatement &decl)
{
  ...
  if (auto decl_search = variable_decls_[scope_].find(var_ident);
    ...
    if (decl_search->second != decl.loc) {
      // Note this will change with proper block/lexical scoping
      LOG(ERROR, decl.loc, err_)
  ...
  variables_[scope_].insert(
      { var_ident, { decl.var->type, can_resize, false } });
...
void SemanticAnalyser::visit(Subprog &subprog)
{
  ...
  for (SubprogArg *arg : subprog.args) {
    variables_[scope_].insert({ arg->name(), { arg->type, true, true } });
  }
