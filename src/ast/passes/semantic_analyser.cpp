...
void SemanticAnalyser::visit(Map &map)
{
  ...
  if (initializer) {
    auto found = BPF_MAP_TYPES.find(initializer->bpf_type);
    if (found == BPF_MAP_TYPES.end()) {
      auto &err = initializer->addError();
      err << "Invalid bpf map type: " << initializer->bpf_type;
