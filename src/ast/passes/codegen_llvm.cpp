...
private:
  ...
  ScopedExpr kstack_ustack(const SizedType &stype, const Location &loc);
...
ScopedExpr CodegenLLVM::kstack_ustack(const SizedType &stype,
                                      const Location &loc)
{
  ...
  auto *stack = b_.CreateKUStackAllocation(stype, stype.stack_type.name(), loc);
  ...
  if (!stype.stack_type.kernel) {
    ...
    b_.CreateStore(b_.GetIntSameSize(get_probe_id(),
                                     stack_struct_type->getTypeAtIndex(3)),
                   b_.CreateGEP(stack_struct_type,
                                stack,
...
ScopedExpr CodegenLLVM::visit(Builtin &builtin)
{
  if (builtin.ident == "nsecs") {
    ...
    return kstack_ustack(builtin.builtin_type, builtin.loc);
...
ScopedExpr CodegenLLVM::visit(Call &call)
{
  if (call.func == "count") {
    ...
    return kstack_ustack(call.return_type, call.loc);
