...
ScopedExpr CodegenLLVM::binop_ptr(Binop &binop)
{
  ...
  if (compare) {
    if (!other_ty.IsPtrTy()) {
      ...
      ptr_expr = b_.CreatePtrToInt(ptr_expr, b_.GetType(other_ty));
    }
    switch (binop.op) {
      case Operator::EQ:
        return ScopedExpr(b_.CreateICmpEQ(ptr_expr, other_expr));
      case Operator::NE:
        return ScopedExpr(b_.CreateICmpNE(ptr_expr, other_expr));
      case Operator::LE: {
        return ScopedExpr(b_.CreateICmpULE(ptr_expr, other_expr));
      }
      case Operator::GE: {
        return ScopedExpr(b_.CreateICmpUGE(ptr_expr, other_expr));
      }
      case Operator::LT: {
        return ScopedExpr(b_.CreateICmpULT(ptr_expr, other_expr));
      }
      case Operator::GT: {
        return ScopedExpr(b_.CreateICmpUGT(ptr_expr, other_expr));
      }
      default:
                                                    ...
                                                    const SizedType &elem_type)
{
  ...
  if (elem_type.IsIntegerTy() || elem_type.IsPtrTy() || elem_type.IsBoolTy()) {
    ...
    return ScopedExpr(b_.CreateDatastructElemLoad(elem_type, src, true));
  } else {
                                                ...
                                                const std::string &temp_name)
{
  ...
  if (elem_type.IsRecordTy() || elem_type.IsArrayTy()) {
    ...
    if (data_type.IsCtxAccess()) {
      ...
      Value *expr = b_.CreateDatastructElemLoad(elem_type, src, true);
