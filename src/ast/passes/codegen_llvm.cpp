                                ...
                                const location &loc)
{
  ...
  Value *condition = b_.CreateICmpSGE(stack_size, b_.getInt32(uint64_size));
  ...
  Value *stack_len = b_.CreateUDiv(stack_size, b_.getInt32(uint64_size));
  // uint64 *stack, uint64 len, uint64 seed
  AllocaInst *seed = b_.CreateAllocaBPF(CreateUInt64(), "seed");
  // use the pid as the seed if we're getting user stacks
  if (is_ustack) {
    b_.CreateStore(b_.CreateGetPidTgid(loc), seed);
  } else {
    b_.CreateStore(b_.getInt64(1), seed);
  }

  // LLVM-12 produces code that fails the BPF verifier because it
  // can't determine the bounds of stack_len. The only thing that seems
  // to work is truncating the type, which is fine because 255 is long enough.
  Value *trunc_stack_len = b_.CreateTrunc(stack_len, b_.getInt8Ty());
  ...
  Value *murmur_hash_2 = b_.CreateCall(murmur_hash_2_func_,
                                       { ptr_to_stack_trace,
                                         trunc_stack_len,
                                         b_.CreateLoad(b_.getInt64Ty(), seed) },
                                       "murmur_hash_2");
  b_.CreateDebugOutput("Stack id %llu. Len: %llu",
                       std::vector<Value *>{ murmur_hash_2, stack_len },
                       loc);
  ...
  if (bpftrace_.debug_output_)
    b_.CreateDebugOutput("Failed to get stack.", std::vector<Value *>{}, loc);
  b_.CreateStore(b_.getInt64(0), stackid);
  b_.CreateBr(merge_block);
...
void CodegenLLVM::generate_maps(const RequiredResources &resources)
{
  ...
  for (const StackType &stack_type : resources.stackid_maps) {
    ...
    if (stack_type.limit > max_stack_limit) {
      max_stack_limit = stack_type.limit;
    }
  }
...
Function *CodegenLLVM::createMurmurHash2Func()
{
  ...
  //
  // uint64_t murmur_hash_2(void *stack, uint8_t len, uint64_t seed) {
  ...
  AllocaInst *len_addr = b_.CreateAllocaBPF(b_.getInt8Ty(), "len_addr");
  ...
  b_.CreateStore(callback->getArg(1), len_addr);
  ...
  auto *cond = b_.CreateICmp(CmpInst::ICMP_ULT,
                             ...
                             b_.CreateLoad(b_.getInt8Ty(), len_addr),
                             "length.cmp");
  ...
  b_.CreateRet(b_.CreateLoad(b_.getInt64Ty(), id));
