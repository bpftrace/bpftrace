      ...
      debug_(*module_)
{
  ...
  license_var->addDebugInfo(debug_.createGlobalString("LICENSE", license));
}
  ...
  if (auto *func = call.function) {
    ...
    if (genedFucns.find(call.func) == genedFucns.end()) {
      ...
      for (const auto &param : func->params()) {
        funcaa_params.push_back(b_.GetType(param.type(), false));
      }

      FunctionType *funcaa_type = FunctionType::get(
          b_.GetType(func->returnType(), false), funcaa_params, false);
      Function *funcaa = Function::Create(
          funcaa_type, Function::ExternalLinkage, func->name(), module_.get());

      // TODO our elf files contain too many sections (no need for .debug*,
      // .rel.debug*, .rel.BTF)
                                   ...
                                   const location &loc)
{
  ...
  size_t expr_size = expr->type.GetSize();

  if (inBpfMemory(expr->type)) {
    if (!expr->type.IsSameSizeRecursive(map.key_type)) {
      b_.CreateMemsetBPF(offset_val, b_.getInt8(0), map_key_size);
      if (expr->type.IsTupleTy()) {
        createTupleCopy(expr->type, map.key_type, offset_val, expr_);
      } else if (expr->type.IsStringTy()) {
        b_.CreateMemcpyBPF(offset_val, expr_, expr_size);
      } else {
        LOG(BUG) << "Type size mismatch. Key Type Size: "
...
BpfBytecode CodegenLLVM::emit(bool dis)
{
  ...
  const auto &imports = bpftrace_.imports();
  if (imports.empty())
  ...
  int newfd = memfd_create("linked_output", 0);
  if (newfd < 0) {
    LOG(ERROR) << "Could not create memfd: " << strerror(errno);
    // TODO throw here
  }
  struct bpf_linker *linker = bpf_linker__new_fd("memfd:linked_output",
                                                 newfd,
                                                 nullptr);
  if (!linker) {
    // TODO error here
  ...
  if (bpf_linker__add_buf(
          linker, "bpftrace_bytecode", output.data(), output.size(), nullptr)) {
    LOG(BUG) << "Generated bytecode not suitable for linking";
  }

  for (const auto &import : imports) {
    if (bpf_linker__add_file(linker, import.path().c_str(), nullptr)) {
      LOG(ERROR) << "Could not link against lib: " << import.name();
      // LOG(HINT) << "View detailed error messages with '-d libbpf'";
      //  TODO throw here
    }
  }

  if (bpf_linker__finalize_fd(linker)) {
    // TODO error
  }
