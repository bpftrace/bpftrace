...
void CodegenLLVM::visit(FieldAccess &acc)
{
  ...
  if (type.is_funcarg) {
    auto probe_type = probetype(current_attach_point_->provider);
    if (probe_type == ProbeType::fentry || probe_type == ProbeType::fexit)
      expr_ = b_.CreateKFuncArg(getContext(), acc.type, acc.field);
  ...
  if (inBpfMemory(type)) {
    ...
    if (field.type.IsIntTy() &&
      ...
      if (field.bitfield.has_value()) {
        ...
        if (type.IsCtxAccess()) {
          // We preserve pointer semantics. See `probereadDatastructElem` where
          // the logic is essentially constrained in the same way.
          if (expr_->getType() != b_.GET_PTR_TY()) {
            if (!expr_->getType()->isPointerTy())
              expr_ = b_.CreateIntToPtr(expr_, b_.GET_PTR_TY());
            else
              expr_ = b_.CreatePointerCast(expr_, b_.GET_PTR_TY());
        ...
        Value *wrapped_ctx = b_.preserveStaticOffset(
            b_.CreatePointerCast(ctx_, b_.getInt32Ty()->getPointerTo()));
        ...
        expr_ = b_.CreateGEP(b_.getInt32Ty(), wrapped_ctx, expr_);
                                          ...
                                          const std::string &temp_name)
{
  ...
  // offsets during access.  The solution to this is to have the local types
  ...
  if (src_data->getType() != b_.GET_PTR_TY()) {
    if (!src_data->getType()->isPointerTy())
      src_data = b_.CreateIntToPtr(src_data, b_.GET_PTR_TY());
    else
      src_data = b_.CreatePointerCast(src_data, b_.GET_PTR_TY());
