...
ScopedExpr CodegenLLVM::visit(Call &call)
{
  if (call.func == "count") {
    ...
    //   } else {
    // #if defined(MIN)
    //     bucket_value->value = min(bucket_value->value, value);
    // #else
    //     bucket_value->value = max(bucket_value->value, value);
    // #endif
    //   }
    //   bucket_value->meta = 1;
    // #elif defined(AVG)
