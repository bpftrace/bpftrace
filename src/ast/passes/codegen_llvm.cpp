...
void CodegenLLVM::visit(Call &call)
{
  if (call.func == "count") {
    ...
    // Some map types (BPF_MAP_TYPE_(PERCPU_)ARRAY from those that we use) do
    // not implement per-cpu counters and bpf_map_sum_elem_count would always
...
// for keyless maps (e.g. @x or @++) of BPF_MAP_TYPE_(PERCPU_)ARRAY type.
...
Function *CodegenLLVM::DeclareKernelFunc(const std::string &func_name)
{
  ...
  for (auto &field : maybe_func_type->fields) {
    if (field.name != "$retval")
      args.push_back(b_.GetType(field.type, false));
  }

  FunctionType *func_type = FunctionType::get(
      b_.GetType(maybe_func_type->GetField("$retval").type, false),
      args,
      false);
  ...
  debug_.createFunctionDebugInfo(
      *fun, maybe_func_type->GetField("$retval").type, debug_args, true);
