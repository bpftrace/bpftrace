...
Pass CreatePidFilterPass()
{
  auto fn = [](PassContext &ctx) {
    auto pid_filter = PidFilterAnalyser(ctx.ast_ctx, ctx.b);
    pid_filter.analyse();
...
void PidFilterAnalyser::analyse()
{
  visit(ctx_.root);
}

void PidFilterAnalyser::visit(Probe &probe)
{
...
// been generated
bool PidFilterAnalyser::probe_needs_pid_filter(AttachPoint *ap)
{
...
Statement *PidFilterAnalyser::add_pid_filter(const location &loc)
{
