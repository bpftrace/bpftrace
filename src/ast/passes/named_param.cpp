...
void NamedParamPass::visit(Expression &expr)
{
  ...
  if (call->vargs.size() == 3) {
    description = call->vargs.at(2).as<String>();
    if (!description) {
      call->vargs.at(2).node().addWarning()
          << "Function 'getopt' requires that the third parameter be a string"
          << " literal to describe the parameters.";
      return;
    }
  }
  ...
  if (used_args.contains(arg_name->value)) {
    if (used_args.at(arg_name->value).gvar != np_default) {
      std::string pre_value;
      if (std::holds_alternative<std::string>(
              used_args.at(arg_name->value).gvar)) {
        pre_value = std::get<std::string>(used_args.at(arg_name->value).gvar);
      } else if (std::holds_alternative<int64_t>(
                     used_args.at(arg_name->value).gvar)) {
        pre_value = std::to_string(
            std::get<int64_t>(used_args.at(arg_name->value).gvar));
      } else if (std::holds_alternative<uint64_t>(
                     used_args.at(arg_name->value).gvar)) {
        pre_value = std::to_string(
            std::get<uint64_t>(used_args.at(arg_name->value).gvar));
      } else {
        pre_value = std::get<bool>(used_args.at(arg_name->value).gvar)
                        ? "true"
                        : "false";
      }
      call->addError()
    ...
    if (description &&
        used_args.at(arg_name->value).description != description->value) {
