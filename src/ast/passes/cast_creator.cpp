                      ...
                      const SizedType &target_type)
{
  ...
  if ((expr_type.IsIntegerTy() || expr_type.IsAggIntMapTy()) &&
      (target_type.IsIntegerTy() || target_type.IsAggIntMapTy())) {
...
void CastCreator::visit(MapAccess &acc)
{
  ...
  if (!try_element_cast(ctx_, acc.key, expr_type, key_type)) {
    LOG(WARNING) << expr_type << " " << key_type;
    LOG(BUG) << "Map keys of different types should always be castable at this "
                "point";
  }
}
