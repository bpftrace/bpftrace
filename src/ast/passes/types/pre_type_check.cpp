...
private:
  Probe *require_probe(Builtin &builtin);

  BPFtrace &bpftrace_;
  Node *top_level_node_ = nullptr;
};

Probe *BuiltinPreCheck::require_probe(Builtin &builtin)
{
  auto *probe = dynamic_cast<Probe *>(top_level_node_);
  if (probe == nullptr) {
    builtin.addError() << "Builtin " << builtin.ident
                       << " not supported outside probe";
  }
  return probe;
}
...
void BuiltinPreCheck::visit(Builtin &builtin)
{
  if (builtin.ident == "ctx") {
    auto *probe = require_probe(builtin);
    if (probe == nullptr)
      return;
