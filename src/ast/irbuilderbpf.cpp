...
void IRBuilderBPF::CreateSkboutput(Value *skb, Value *len, AllocaInst *data, size_t size, int caplen)
{
  Value *flags, *map_ptr, *size_val;

  map_ptr = CreateBpfPseudoCallId(bpftrace_.maps[MapManager::Type::PerfEvent].value()->id);

  flags = getInt64(0);
  flags = CreateAdd(flags, len);
