                                         ...
                                         const Location &loc)
{
  ...
  // Tell LLVM if a BPF helper is a pure function (i.e. has no side effects and
  // always returns the same value when called repeatedly within a single
  // probe's context), so that it can optimise out repeated calls to the same
  // helper.
  if (is_pure)
