...
using PassFPtr = std::function<PassResult(Program &, PassContext &)>;
...
public:
  ...
  PassResult Run(Program &program, PassContext &ctx)
  {
    return fn_(program, ctx);
  };
...
public:
  ...
  void AddPass(Pass p);
  [[nodiscard]] PassResult Run(Program &program, PassContext &ctx);
