...
public:
  ...
  // Furthermore, the expansion will only work with `Result<T>` or `Error`
  ...
  static Pass create(
      ...
      std::function<Result<Return>(ASTContext &, Inputs &...)> fn)
  {
    ...
    return Pass(name,
                std::move(inputs),
                std::move(outputs),
                [fn](ASTContext &ast, PassContext &ctx) -> Result<> {
                     ...
                     std::function<Return(Inputs...)> fn)
  {
    ...
    if constexpr (std::is_same_v<Return, void>) {
      return create(name, [fn](Inputs... inputs) -> Result<OK> {
        fn(inputs...);
        return OK();
      });
    } else {
      return create(name, [fn](Inputs... inputs) -> Result<Return> {
        return fn(inputs...);
      });
    }
  }
  ...
  // well-formed (e.g. does not return `Result`, etc.) then this will still
  // fail to instantiate.
  ...
  Result<> run(ASTContext &ast, PassContext &ctx) const
  {
    return fn_(ast, ctx);
  }
...
private:
  ...
  using Fn = std::function<Result<>(ASTContext &ast, PassContext &ctx)>;
...
public:
  ...
  {
    return add(
        Pass::create(T::type_name(), [&t]([[maybe_unused]] ASTContext &ast) {
          return std::ref(t);
        }));
  }
  ...
  Result<PassContext> run(ASTContext &ast)
  {
    PassContext ctx;
    auto err = foreach([&](auto &pass) -> Result<> {
      if (!ast.diagnostics().ok())
        return OK(); // Skip the pass.
