...
struct KernelHeaderTmpDir {
  ...
  ~KernelHeaderTmpDir()
  {
    if (path.size() > 0) {
      ...
      // ourselves
      std_filesystem::remove_all(path);
    }
  }

  void move_to(const std::string &new_path)
  {
    std_filesystem::remove_all(new_path);
...
std::string unpack_kheaders_tar_xz(const struct utsname &utsname)
{
  ...
  struct stat st;
  if (stat(shared_path.c_str(), &st) == 0) {
    if (st.st_uid != 0) {
      LOG(FATAL) << "header ownership expected to be root: " << shared_path
                 << ". Delete this directory and re-run. It will be "
                    "re-generated by bpftrace.";
    }
