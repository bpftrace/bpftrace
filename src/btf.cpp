    ...
    const util::KernelFunctionInfo &func_info)
{
  ...
  if (check_traceable) {
    if (!func_info.is_traceable(std::string(func))) {
      if (func_info.get_traceable_funcs().empty()) {
        return make_error<ast::ArgParseError>(
            func,
...
std::string BTF::get_all_funcs_from_btf(const BTFObj &btf_obj) const
{
  ...
  for (; id <= max; id++) {
    ...
    // Note: Traceable function filtering has been removed from BTF.
    // The caller should filter the list if needed.
...
SizedType BTF::get_stype(std::string_view type_name)
{
  ...
  if (type_name.ends_with(user_pointer)) {
    auto pointee = std::string(
        type_name.substr(0, type_name.length() - sizeof(user_pointer)));
    util::rtrim(pointee);
