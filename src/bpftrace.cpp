...
void perf_event_printer(void *cb_cookie, void *data, int size)
{
  ...
  if (printf_id == AsyncAction::exit) {
    ...
    // Attach the real watchpoint probe
    {
      ...
      std::vector<std::unique_ptr<AttachedProbe>> aps = bpftrace->attach_probe(
          wp_probe, bpftrace->bytecode_);
      ...
      for (auto &ap : aps) {
        if (!ap->did_attach()) {
          const auto &err_msg = ap->err_msg();
          if (bpftrace->config_->missing_probes == ConfigMissingProbes::error) {
            if (!err_msg.empty()) {
              LOG(ERROR) << err_msg;
            }
            LOG(ERROR) << "Unable to attach probe: " << ap->probe().name << ".";
            abort = true;
          } else if (bpftrace->config_->missing_probes ==
                     ConfigMissingProbes::warn) {
            if (!err_msg.empty()) {
              LOG(WARNING) << err_msg;
            }
            LOG(WARNING) << "Unable to attach probe: " << ap->probe().name
                         << ". Skipping.";
          }
        }
...
int BPFtrace::run(BpfBytecode bytecode)
{
  ...
  // This is a little bit of a lie as we don't actually attach the END
  // special probe until the end
  ...
  for (const auto &ap : attached_probes_) {
    if (ap->did_attach()) {
      ++num_attached;
    } else {
      const auto &err_msg = ap->err_msg();
  ...
  if (config_->missing_probes == ConfigMissingProbes::error &&
      num_attached != attached_probes_.size()) {
    return -1;
  }
  ...
  if (!bt_quiet)
    out_->attached_probes(total_attached);
