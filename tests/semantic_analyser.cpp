...
kprobe:f { fail("always fail"); }
  ...
  test(
      R"(kprobe:f { $x = 1; if (typeid($x) != typeid(1)) { fail("only integers"); } })");
  test(
      R"(kprobe:f { $x = 1; if (typeid($x) == typeid(1)) { fail("no integers"); } })",
      Error{ R"(
stdin:1:51-72: ERROR: only integers
kprobe:f { $x = 1; if (typeid($x) != typeid(1)) { fail("no integers"); } }
                                                  ~~~~~~~~~~~~~~~~~~~~~
)" });
}
...
TEST_F(SemanticAnalyserTest, typevalid_if_constexpr)
{
  ...
  test(
      R"(kprobe:f { $x = (int8*)0; if (!typevalid($x[0])) { fail("pointer can't be indexed"); } })");
}
