...
{
  test("BEGIN { $a = getopt(\"hello\"); }", "named param: hello");
  test("BEGIN { $a = getopt(\"hello\", 1); }", "named param: hello");
  test("BEGIN { $a = getopt(\"hello\", true); }", "named param: hello");
  test("BEGIN { $a = getopt(\"hello\", false); }", "named param: hello");
  test("BEGIN { $a = getopt(\"hello\", on); }", "named param: hello");
  test("BEGIN { $a = getopt(\"hello\", off); }", "named param: hello");
  test("BEGIN { $a = getopt(\"hello\", \"bye\"); }", "named param: hello");
  ...
  test_error("BEGIN { $a = getopt(\"hello\", banana); }",
             ...
             "no).");
  test_error(
      "BEGIN { $a = getopt(\"hello\", 1); $b = getopt(\"hello\", \"bye\"); }",
