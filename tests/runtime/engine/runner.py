...
class Runner(object):
    ...
    def prepare_bpf_call(test, nsenter=[]):
        ...
        if test.run:
            ...
            use_json = (
                "-q -f json"
                if (len(test.expects) > 0 and test.expects[0].mode == "json")
                else ""
            )
            escaped_prog = test.prog.replace("'", "'\\''")
            cmd = nsenter_prefix + "{} {} -e '{}'".format(
                BPFTRACE_BIN, use_json, escaped_prog
            )
    ...
    def __get_bpffeature():
        ...
        bpffeature["libpath_resolv"] = (
            output.find("bcc library path resolution: yes") != -1
        )
        ...
        with open(os.devnull, "w") as dn:
            ...
            while waited <= timeout:
                ...
                children = subprocess.run(
                    run_cmd, check=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE
                )
                if children.returncode == 0 and children.stdout:
                    lines = [
                        line.decode("utf-8") for line in children.stdout.splitlines()
                    ]
                    if condition(lines):
                        return lines
                else:
                    print(
                        f"__wait_for_children error: {children.stderr}. Return code: {children.returncode}"
                    )
    ...
    def run_test(test):
        ...
        if (
            test.skip_if_env_has
            and os.environ.get(test.skip_if_env_has[0], "") == test.skip_if_env_has[1]
        ):
            print(warn("[   SKIP   ] ") + "%s.%s" % (test.suite, test.name))
        ...
        def check_expect(expect, output):
            try:
                if expect.mode == "text":
                    ...
                    return re.search(
                        f"^\\s*{re.escape(expect.expect)}\\s*$", output, re.M
                    )
                    ...
                    with open(expect.expect) as expect_file:
                        ...
                        if file_extension == ".ndjson":
                            ...
                            for x in range(len(file_lines)):
                                if json.loads(output_lines[x]) != json.loads(
                                    file_lines[x]
                                ):
                                    return False
                        ...
                        if len(output_lines) != 1:
                            print(
                                f"Expected a single line of json ouput. Got {len(output_lines)} lines"
                            )
                            return False
        ...
        try:
            ...
            if test.new_pidns and not test.befores:
                raise ValueError(
                    "`NEW_PIDNS` requires at least one `BEFORE` directive as something needs to run in the new pid namespace"
                )
            ...
            if test.befores:
                if test.new_pidns:
                    ...
                    lines = Runner.__wait_for_children(
                        unshare_out.pid,
                        test.timeout,
                        "pid",
                        lambda lines: len(lines) > 0,
                    )

                    if not lines:
                        raise TimeoutError(
                            f"Timed out waiting create a new PID namespace"
                        )
                    ...
                    for before in test.befores[1:]:
                        ...
                        if not Runner.__wait_for_children(
                            before.pid,
                            test.timeout,
                            "comm",
                            lambda lines: child_name in lines,
                        ):
                    ...
                    child_names = [
                        os.path.basename(x.strip().split()[-1]) for x in test.befores
                    ]
                    child_names = sorted(
                        (x[:15] for x in child_names)
                    )  # cut to comm length

                    def found_all_children(lines):
                        return (
                            sorted((line.strip() for line in lines if line != "ps"))
                            == child_names
                        )

                    if not Runner.__wait_for_children(
                        os.getpid(), test.timeout, "comm", found_all_children
                    ):
                        raise TimeoutError(
                            f"Timed out waiting for BEFORE(s) {test.befores}"
                        )
            ...
            if test.befores and "{{BEFORE_PID}}" in bpf_call:
                ...
                if len(test.befores) > 1:
                    raise ValueError(
                        f"test has {len(test.befores)} BEFORE clauses but BEFORE_PID usage requires exactly one"
                    )
            ...
            while p.poll() is None:
                ...
                if not attached and nextline == "__BPFTRACE_NOTIFY_PROBES_ATTACHED\n":
                    ...
                    if test.after:
                        after_cmd = (
                            get_pid_ns_cmd(test.after) if test.new_pidns else test.after
                        )
        ...
        if (
            p
            and p.returncode != test.return_code
            and not test.will_fail
            and not timeout
        ):
            ...
            for failed_expect in failed_expects:
                if failed_expect.mode == "text":
                    ...
                    if file_extension == ".ndjson":
                        ...
                        try:
                            expected = json.dumps(
                                json.loads(open(failed_expect.expect).read()), indent=2
                            )
                        except json.decoder.JSONDecodeError as err:
                            expected = (
                                "Could not parse JSON: "
