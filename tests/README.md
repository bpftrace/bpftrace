...
* To run only specific self tests, use `--probe-filter REGEX` to select tests by name, e.g. `<builddir>src/bpftrace --test --probe-filter 'map_' <builddir>/tests/<testfile>`. When running `self-tests.sh`, use `--filter` to run a subset of tests, implemented using the `bpftrace --probe-filter` parameter.
