...
public:
  UnopMatcher& IsPostOp(bool is_post_op = true)
  {
    return Where([is_post_op](const ast::Unop& node) {
      return node.is_post_op == is_post_op ||
             (node.addError()
                  << "is " << (node.is_post_op ? "post-op" : "pre-op")
                  << " instead of " << (is_post_op ? "post-op" : "pre-op"),
              false);
    });
  }
};
...
{
  return UnopMatcher().WithOperator(op).WithExpr(expr).IsPostOp(is_post_op);
}
