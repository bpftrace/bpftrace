...
TEST(Parser, builtin_variables)
{
  ...
  test("kprobe:f { sarg0 }",
       Program().WithProbe(
           Probe({ "kprobe:f" }, { ExprStatement(Builtin("sarg0")) })));
...
TEST(Parser, variable_post_increment_decrement)
{
  test("kprobe:sys_open { $x++; }",
       Program().WithProbe(
           Probe({ "kprobe:sys_open" },
                 { ExprStatement(
                     Unop(Operator::INCREMENT, Variable("$x"), true)) })));

  test("kprobe:sys_open { ++$x; }",
       Program().WithProbe(
           Probe({ "kprobe:sys_open" },
                 { ExprStatement(
                     Unop(Operator::INCREMENT, Variable("$x"), false)) })));

  test("kprobe:sys_open { $x--; }",
       Program().WithProbe(
           Probe({ "kprobe:sys_open" },
                 { ExprStatement(
                     Unop(Operator::DECREMENT, Variable("$x"), true)) })));

  test("kprobe:sys_open { --$x; }",
       Program().WithProbe(
           Probe({ "kprobe:sys_open" },
                 { ExprStatement(
                     Unop(Operator::DECREMENT, Variable("$x"), false)) })));
}
...
TEST(Parser, map_increment_decrement)
{
  test("kprobe:sys_open { @x++; }",
       Program().WithProbe(Probe(
           { "kprobe:sys_open" },
           { ExprStatement(Unop(Operator::INCREMENT, Map("@x"), true)) })));

  test("kprobe:sys_open { ++@x; }",
       Program().WithProbe(Probe(
           { "kprobe:sys_open" },
           { ExprStatement(Unop(Operator::INCREMENT, Map("@x"), false)) })));

  test("kprobe:sys_open { @x--; }",
       Program().WithProbe(Probe(
           { "kprobe:sys_open" },
           { ExprStatement(Unop(Operator::DECREMENT, Map("@x"), true)) })));

  test("kprobe:sys_open { --@x; }",
       Program().WithProbe(Probe(
           { "kprobe:sys_open" },
           { ExprStatement(Unop(Operator::DECREMENT, Map("@x"), false)) })));
}
       ...
       Program().WithProbe(Probe(
           ...
           { AssignVarStatement(Variable("$a"), Integer(0)),
             While(Binop(Operator::LT, Variable("$a"), Integer(10)),
                   { ExprStatement(
                       Unop(Operator::INCREMENT, Variable("$a"), true)) }) })));
}
