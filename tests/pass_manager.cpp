...
{
  return Pass::create("test1", [error](ASTContext &) -> Result<> {
    if (error) {
...
{
  return Pass::create("test2", [error](ASTContext &) -> Result<Test2Output> {
    if (error) {
...
{
  return Pass::create("test3",
                      [](ASTContext &, Test2Output &) -> Result<Test3Output> {
                        return Test3Output();
                      });
}
...
TEST(PassManager, noop_pass)
{
  ast::ASTContext ast;
  PassManager pm;
  pm.add(Pass::create("void", [](ASTContext &) {}));
  EXPECT_TRUE(bool(pm.run(ast)));
}
